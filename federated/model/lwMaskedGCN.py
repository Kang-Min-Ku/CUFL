...
class GCN(nn.Module):
    def __init__(self, num_feat=10, num_dims=128, num_class=10, args=None):
        super().__init__()
        ...
        self.conv1 = GCNConv(self.num_feat, self.num_dims, cached=False)
        self.conv2 = GCNConv(self.num_dims, self.num_dims, cached=False)
        self.clsif = nn.Linear(self.num_dims, self.num_class)
...
class MaskedGCN(nn.Module):
    def __init__(self, num_feat=10, num_dims=128, num_class=10, l1=1e-3, args=None):
        super().__init__()
        self.args = args
        self.num_feat = num_feat
        self.num_dims = num_dims
        ...
        for i in range(self.num_layers):
            if i == 0:
                self.convs[str(i)] = MaskedGCNConv(self.num_feat, self.num_dims, args=self.args)
            else:
                self.convs[str(i)] = MaskedGCNConv(self.num_dims, self.num_dims, args=self.args)

        if self.args.use_classifier_mask:
            self.classifier = MaskedLinear(self.num_dims, self.num_class, l1=l1, args=self.args)
        else:
    ...
    def forward(self, data, get_feature=False):
        ...
        if get_feature:
            return features
        x = self.classifier(x)
        # unify output format with other models
        return x 
...
class MaskedGCNConv(MessagePassing):
    ...
    def __init__(self, in_channels: int, out_channels: int,
        ...
        self.l1 = l1
        self.mask_one_init = args.maskedgcn["laye_mask_one"]
        self.mask_rank = args.mask_rank
        self.mask_drop = args.maskedgcn["mask_drop"]
        self.mask_drop_ratio = args.maskedgcn["mask_drop_ratio"]
        self.mask_noise = args.maskedgcn["mask_noise"]
