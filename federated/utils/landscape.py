...
class hessian():
    ...
    def __init__(self, model, criterion, data=None, dataloader=None, cuda=True, gpu_id=None):
        """
...
def get_loss_landscape(model,
                       ...
                       gpu_id=None):
    if lambda1 is None:
        lambda1 = np.linspace(-0.5,0.5,21).astype(np.float32)
    if lambda2 is None:
        lambda2 = np.linspace(-0.5,0.5,21).astype(np.float32)
    if gpu_id is not None:
        data = data.cuda(gpu_id)
        model = model.cuda(gpu_id)
    model_perb1 = model_perb1.cuda(gpu_id)
    model_perb2 = model_perb2.cuda(gpu_id)
    ...
    y = data.y
    hessian_comp = hessian(model, criterion, data=(data, y), cuda=True, gpu_id=gpu_id)
    ...
    for l1 in lambda1:
        for l2 in lambda2:
            model_perb1 = get_params(model, model_perb1, top_eigenvectors[0], l1)
            model_perb2 = get_params(model, model_perb2, top_eigenvectors[1], l2)
            loss_list.append((l1,l2,criterion(model_perb2(data),y).item()))
